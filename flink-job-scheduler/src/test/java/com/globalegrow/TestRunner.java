...
public class TestRunner extends AbstractFlinkJobSerialScheduler {
    ...
    String commandLine2 = "/usr/local/services/flink/flink-yarn/flink-1.5.0/bin/flink run -d -m " +
            "yarn-cluster -yn 1 -yjm 1024 -ytm 1024 -yqu root.flink -ynm " +
            "gb-app-user-event-init-everyday /usr/local/services/flink/hdfs-orc-file-es-0.1.jar " +
            "--elastic-servers 172.31.47.84:9302,172.31.43.158:9302,172.31.55.231:9302 " +
            "--index-name dy_app_gb_event --type-name log --cluster.name esearch-aws-dy --es.eventIndex true " +
            "--es.routing true --hdfs.server hdfs://172.31.20.96:8020 " +
            "--filePath hdfs://172.31.20.96:8020/bigdata/ods/log_clean/ods_app_burial_log/2019/10/08/gearbest " +
            "--query.sql \\\"select event_name,event_value,appsflyer_device_id,customer_user_id," +
            "event_time,app_name,platform from user_event where event_name in('af_view_product','af_add_to_bag','af_add_to_wishlist','af_create_order_success','af_search','af_purchase') and lower(app_name) like '%gearbest%'\\\" " +
            "--job.parallelism 1";
