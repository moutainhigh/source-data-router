...
public class DataLocalBuffer {
    ...
    protected static final Logger logger = LoggerFactory.getLogger(DataLocalBuffer.class);

    //@Async
    public void handleMsgAsync(String logString, LinkedBlockingDeque<Map> linkedBlockingDeque) {
    ...
    public List<Map> logWriteToRedis(String logString) {
        if (logString.contains("/_app.gif?")) {
            try {
                ...
                /*Index index = new Index.Builder(map).index("bts-app-dopamine-" + DateFormatUtils.format((Long) map.get(NginxLogConvertUtil.TIMESTAMP_KEY), "yyyy-MM-dd")).type("log").build();
